...
class Purchase_requisition_request extends Root_Controller
{
    ...
    private function system_get_items()
    {
        ...
        $this->db->join($this->config->item('table_login_setup_user').' user','user.id=item.user_id','INNER');
        //$this->db->select('user.id,user.employee_id,user.user_name,user.status');

        $this->db->join($this->config->item('table_login_setup_user_info').' user_info','user.id = user_info.user_id','INNER');
        $this->db->select("concat_ws(' - ',user_info.name,user.employee_id) user_name");
    ...
    private function system_add()
    {
        if(isset($this->permissions['action1'])&&($this->permissions['action1']==1))
        {
            ...
            $this->db->select('user.id,user.employee_id,user.user_name,user.status');
            $this->db->join($this->config->item('table_login_setup_user_info').' user_info','user.id = user_info.user_id','INNER');
            $this->db->select("concat_ws(' - ',user_info.name,user.employee_id) name,user_info.ordering,user_info.blood_group,user_info.mobile_no");
            $this->db->where('user.status',$this->config->item('system_status_active'));
            $this->db->where('user_info.revision',1);
    ...
    private function check_validation()
    {
        ...
        $this->form_validation->set_rules('item[user_id]',$this->lang->line('LABEL_NAME'),'required');
