...
class Setup_responsible_user_group extends Root_Controller
{
    ...
    public function index($action="list",$id=0)
    {
        ...
        elseif($action=="add_edit_user_assign")
        {
            $this->system_add_edit_user_assign($id);
        }
        elseif($action=="save_user_assign")
        {
            $this->system_save_user_assign();
        }
    ...
    private function system_list()
    {
        ...
        {
            $data['system_preference_items']= System_helper::get_preference($user->user_id, $this->controller_url, $method, $this->get_preference_headers($method));
            $data['title']="Responsible User Group List";
            $ajax['status']=true;
    ...
    {
        $this->db->from($this->config->item('table_ams_setup_responsible_user_group').' item');
    ...
    private function system_add()
    {
        ...
        {
            $data['title']="Create New Responsible User Group";
            $data['item']['id']=0;
            $data['item']['name']='';
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $data['item']=Query_helper::get_info($this->config->item('table_ams_setup_responsible_user_group'),array('*'),array('id ='.$item_id),1,0,array('id ASC'));
            if(!$data['item'])
            {
            ...
            $data['title']="Edit Responsible User Group :: ". $data['item']['name'];
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/add_edit",$data,true));
            if($this->message)
            {
    ...
    private function system_save()
    {
        ...
        if($id>0)
        {
            ...
            Query_helper::update($this->config->item('table_ams_setup_responsible_user_group'),$item, array('id='.$id), false);
        }
        else
        {
    ...
    private function system_add_edit_user_assign($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            if($id>0)
            ...
            $this->db->order_by('user_info.ordering');
            $data['users']=$this->db->get()->result_array();

            $data['title']="Assign Responsible User Group By Employee:: ". $data['item']['name'];
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/add_edit_user_assign",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
            $ajax['system_page_url']=site_url($this->controller_url.'/index/add_edit_user_assign/'.$item_id);
            $this->json_return($ajax);
        }
        else
        {
